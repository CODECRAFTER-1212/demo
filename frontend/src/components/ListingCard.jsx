...
export default function ListingCard({ listing }) {
  ...
  const cardContent = (
    <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-md transition-shadow duration-300">
      <div className="aspect-w-4 aspect-h-3 bg-gray-200 relative overflow-hidden">
        ...
        {listing.isDummy && (
          <div className="absolute top-2 left-2 bg-amber-400/90 backdrop-blur-sm px-2 py-1 rounded text-xs font-semibold text-amber-900 shadow-sm">
            Demo
          </div>
        )}
      </div>
  ...
  // All listings (real & dummy) are clickable
  // Dummy listings pass their data via router state so no API call is needed
  return (
    <Link
